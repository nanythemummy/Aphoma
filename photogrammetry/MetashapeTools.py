...
def load_photos(chunk, projectdir:str, photodir:str):
    ...
    LOGGER.info("Loading images from %s", photodir)
    for i in images:
...
def load_masks(chunk,maskpath:str,projectdir:str):
    if not maskpath or not os.path.exists(os.path.join(projectdir,maskpath)):
        LOGGER.warning("Mask path %s doesn't exist or wasn't passed corectly. Failing loading masks.",maskpath)
        return
    maskpath = os.path.join(projectdir,maskpath)
    files = os.listdir(maskpath)
    if len(files)>0:
        LOGGER.info("Loading masks from %s", maskpath)
...
def build_basic_model(photodir:str, projectname:str, projectdir:str, config:dict, decimate = True, maskoption = 0):
    ...
    """
    LOGGER.info('Building model %s in location %s from photos in %s', projectname, photodir,projectdir)
    ...
    try:
        if os.path.exists(projectpath):
            LOGGER.info("Loading project %s",projectpath)
        ...
        if len(doc.chunks)==0:
            ...
            doc.save()      
            LOGGER.info('Added chunk %s.', projectname)
        else:
            current_chunk=doc.chunks[0]
            LOGGER.info('A Chunk called %s already exists. Building it.', current_chunk.label)
        #build sparse cloud.
        ...
        if "palette" in config.keys() and config["palette"] != "None":
            ...
            LOGGER.info("Finding markers as defined in %s.", config["palette"])
            ...
            if "scalebars" in palette.keys() and not current_chunk.scalebars:
                LOGGER.info("Attempting to define scalebars.")
                ...
                doc.save() 
        #remove blobs.
        LOGGER.info("Cleaning up detached geometry.")
        ...
        if decimate and len(doc.chunks)<2:
            LOGGER.info("Building a lower poly version of the model from a duplicate chunk.")
        ...
        for c in doc.chunks:
            if not c.model.textures:
                LOGGER.info("Building UV Map and Texture for chunk %s",c.label)
        ...
        for c in doc.chunks:
            ...
            if not ext == "none":
                ...
                for extn in outputtypes:
                    ...
                    LOGGER.info("Now, exporting chunk %s as %s",c.label,name )
        ...
        LOGGER.info("Completed model and export in %s seconds.",stoptime-starttime)
...
def reorient_model(chunk,config):
    ...
    if len(axes)==0:
        LOGGER.warning("No axes on which to orient chunk %s",chunk.label)
        return
    else:
        
        LOGGER.info("Reorienting chunk %s according to markers on palette.",chunk.label)
